...
Gemini CLI is available in preview; OpenHands, OpenClaw, and ZeptoClaw remain
unavailable there. Some
...
| `defenseclaw setup geminicli` | Gemini CLI hook setup alias on macOS, Linux, and native Windows preview. Defaults to observe; pass `--mode action` to block on supported hook events. Windows Setup binds `%USERPROFILE%\.gemini` and uses an awaited system-PowerShell launcher. |
