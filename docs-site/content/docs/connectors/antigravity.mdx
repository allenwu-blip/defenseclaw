...
After the integrated certification gate is promoted, native Setup will support:

```powershell
defenseclaw setup antigravity --yes
defenseclaw setup antigravity --mode action --yes
defenseclaw doctor
```
...
The pending native Setup implementation writes five DefenseClaw-owned registrations to
`%USERPROFILE%\.gemini\config\hooks.json`. It backs up the original file,
reconciles stale DefenseClaw commands on repair/upgrade, preserves unrelated
hooks, writes connector-scoped gateway credentials into the protected
DefenseClaw runtime, and records the resolved contract in
...
workspace behavior; rules and plugin-contained agents remain discovery-only as
listed in the connector matrix.
