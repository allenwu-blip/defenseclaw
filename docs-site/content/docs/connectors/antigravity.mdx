...
publish CLI v1.1.9 (July 31, 2026). This is vendor availability metadata, not
...
The current manual delta confirms only that public Setup rejects Antigravity as
`not_certified`. No protected-client, authentication, or client-provenance test
occurred, so it contributes no certification evidence. Shared readiness remains
integration-owned.
...
Doctor never executes command text read from `hooks.json`. Because this preview
remains `not_certified`, public `defenseclaw setup antigravity` and
`defenseclaw doctor --fix` do not repair it. For an existing recorded packaged
installation, run the cached DefenseClaw Setup `/repair` action without a
`CONNECTOR` override; that already-supported maintenance path reloads the
recorded connector and reconciles its native registration.
...
the compatibility floor for the five-event contract. The current v1.1.9
...
connector beyond `not_certified` / `live: false`.
...
policy surfaces are outside this connector preview and remain unverified.
