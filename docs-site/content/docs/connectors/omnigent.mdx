...
client validation completes successfully.
...
| `tool_result` | `PostToolUse` | attributed observation after execution; cannot replace or reverse the result |
| `response` | `AfterAgentResponse` | attributed observation; cannot replace or reverse the response |
| `llm_request` | `BeforeModel` | ALLOW, native ASK, or DENY |
| `llm_response` | `AfterModel` | attributed observation; cannot replace or reverse the response |
