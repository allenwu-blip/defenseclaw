...
def prepare_windows_contract_v8(
    ...
    *,
    ...
    absent or exactly empty observability mapping so this helper can never
    replace an operator-authored destination graph.
    """
    ...
    if observability not in (None, {}):
        raise ValueError(
            "Windows contract requires an absent or empty fresh-v8 observability mapping"
        )
    destination = {
