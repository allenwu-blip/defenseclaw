...
| Antigravity | **OUTDATED / LIMITED:** official manifest latest is 1.1.9; macOS/Linux `>=1.1.8,<1.1.9` correctly rejects it pending review; Windows preserves `>=1.1.8` unbounded; 1.1.8 is the reviewed five-event v2 contract | `preview` | **UNCERTIFIED:** latest macOS is unsupported and no packaged authenticated client record exists |
