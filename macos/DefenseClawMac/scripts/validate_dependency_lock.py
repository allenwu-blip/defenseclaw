...
def authenticated_direct_requirements(
    ...
    requirements.extend(document.get("tool", {}).get("uv", {}).get("override-dependencies", []))
...
def validate_lock(pyproject_path: Path, lock_path: Path) -> int:
    ...
    for stanza in requirement_stanzas(lock_text):
        ...
        for line in stanza[1:]:
            ...
            match = HASH_PATTERN.fullmatch(continuation)
            if match is None:
                fail(f"dependency lock contains an invalid continuation: {requirement_name}")
