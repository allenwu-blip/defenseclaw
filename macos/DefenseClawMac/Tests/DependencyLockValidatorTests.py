...
class DependencyLockValidatorTests(unittest.TestCase):
    ...
    def test_rejects_non_hash_continuation_tokens(self) -> None:
        unexpected_tokens = (
            "--index-url=https://packages.example.invalid/simple",
            "--only-binary=:all:",
            "https://packages.example.invalid/unbound.whl",
        )
        for unexpected in unexpected_tokens:
            with self.subTest(unexpected=unexpected):
                ...
                self.assertNotEqual(result.returncode, 0)
                self.assertIn("invalid continuation", result.stdout)
