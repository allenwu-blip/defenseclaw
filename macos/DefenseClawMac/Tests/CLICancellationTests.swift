...
struct CLICancellationTests {
    ...
    private static func cancelledParentDoesNotLeaveDescendant() async {
        ...
        let childPID = await waitForPID(parentMarker)
        let childStarted = await waitForFile(childMarker)
        expect(childPID != nil, "process-tree descendant PID is recorded")
    ...
    private static func forkedReparentedGrandchildIsKilled() async {
        ...
        let grandchildPID = await waitForPID(grandchildMarker)
        expect(grandchildPID != nil, "signaled worker records the forked grandchild PID")
        guard let grandchildPID else { return }
    ...
    private static func standardInputIsWrittenAndClosed() async {
        ...
        let result = await runner.run(
            ...
            standardInput: AlertDispositionCommand.confirmationInput
        )
        expect(result.succeeded, "stdin-backed command succeeds")
        expect(
            result.output == "y\neof\n",
            "one exact confirmation line is written and stdin reaches EOF"
        )
    }
    ...
    private static func waitForPID(_ url: URL, attempts: Int = 100) async -> pid_t? {
        for _ in 0..<attempts {
